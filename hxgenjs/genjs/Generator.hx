					...
					switch type {
						...
						case PClass(c): 
							switch config.classGenerator.generate(api, c) {
								case Some(code):
                  switch(c.entryPoint) {
                    case true: trace('wow');write(path + 'app/' + c.id.asFilePath() + config.fileExtension, code);
                    case false: trace(path);write(path + c.id.asFilePath() + config.fileExtension, code);
                  }
								case None:
							}
						case PEnum(e): 
